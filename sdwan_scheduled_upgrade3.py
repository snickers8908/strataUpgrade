...
This script:
  ...
  2. Calls GET /sdwan/v2.1/api/profile immediately after auth (required by
     the Unified SASE API or all subsequent calls return 403).
  3. Offers a dry-run (cold run) that shows exactly what will happen —
     no changes are made until you confirm.
  4. Waits until DOWNLOAD_TIME, then triggers a software download on every
     machine in the tenant.
  5. Waits until UPGRADE_TIME, then triggers the upgrade on every machine.
...
# Unified SASE API base — no region needed, OAuth2 token handles routing
...
def refresh_token() -> None:
    """Fetch a new token, store it globally, then prime the SD-WAN session."""
    global _access_token
    _access_token = fetch_access_token()
    _prime_sdwan_session()
...
def _prime_sdwan_session() -> None:
    """
    The Unified SASE SD-WAN API requires a call to GET /sdwan/v2.1/api/profile
    immediately after obtaining a token. Without this, all subsequent SD-WAN
    API calls return 403.
    """
    url  = f"{BASE_URL}/sdwan/v2.1/api/profile"
    resp = requests.get(url, headers=auth_headers(), timeout=30)
    resp.raise_for_status()
    log.info("SD-WAN session primed via /profile (tenant: %s).", TSG_ID)
...
def wait_until(target_str: str) -> None:
    ...
    while True:
        ...
        refresh_token()  # re-auth + re-prime session every wake cycle


# ---------------------------------------------------------------------------
# API helpers  (Unified SASE SD-WAN paths)
# ---------------------------------------------------------------------------

def get_all_machines() -> list[dict]:
    """Return all element/machine objects for the tenant."""
    # Unified API: no tenant_id in path — tenant is inferred from the OAuth2 token
    url  = f"{BASE_URL}/sdwan/v2.0/api/machines"
    resp = requests.get(url, headers=auth_headers(), timeout=30)
    resp.raise_for_status()
...
def get_machine_software(machine_id: str) -> list[dict]:
    """Return software records for a single machine."""
    url  = f"{BASE_URL}/sdwan/v2.0/api/machines/{machine_id}/software"
    resp = requests.get(url, headers=auth_headers(), timeout=30)
    resp.raise_for_status()
...
def get_element_images() -> list[dict]:
    """Return available element images from the controller."""
    url  = f"{BASE_URL}/sdwan/v2.4/api/element_images"
    resp = requests.get(url, headers=auth_headers(), timeout=30)
    resp.raise_for_status()
    return resp.json().get("items", [])
...
def find_image_id(images: list[dict], version: str) -> str | None:
    """Find the image ID that matches *version*."""
    for img in images:
...
def update_machine_software(machine_id: str, software_id: str, payload: dict) -> dict:
    """PUT an updated software record for one machine."""
    url = f"{BASE_URL}/sdwan/v2.0/api/machines/{machine_id}/software/{software_id}"
...
def dry_run(machines: list[dict], image_id: str) -> bool:
    ...
    for machine in machines:
        ...
        try:
            ...
            # Try to find the currently active version
...
def main() -> None:
    ...
    # Authenticate + prime SD-WAN session
    refresh_token()
    ...
    if not images:
        log.warning("WARNING: No images returned from controller. The endpoint may be wrong or your token may lack permissions.")
    else:
        log.info("Images returned by controller:")
        for img in images:
            log.info("  id=%-30s  version=%-20s  name=%s",
                     img.get("id", "-"),
                     img.get("version", "-"),
                     img.get("name", "-"))
    ...
    if not image_id:
        ...
        raise ValueError(
            ...
            f"Check that TARGET_VERSION exactly matches one of the above."
        )
